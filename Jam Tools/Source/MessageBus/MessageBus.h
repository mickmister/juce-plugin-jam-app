...
public:
    ...
    void updateTopic(juce::String name, float value);
    void addListener(juce::String name, BusListener* listener);
...
private:
    ...
    ~MessageBus() {
        for(std::map<juce::String,Topic*>::iterator it = topics.begin(); it != topics.end();++it) {
            delete (*it).second;
        }
        topics.clear();
    }
